...
const insertStudentMethods = async (req, res) => {
    try {
        ...
        const method = await methodModel.insertStudentMethods(studentId,editalId, areaId, methods);
...
const deleteStudentMethods = async (req, res) => {
    try {
        const { studentId,editalId, areaId, methods } = req.body;
        const method = await methodModel.deleteStudentMethods(studentId,editalId, areaId, methods);
        return res.status(200).json(method);
    } catch (error) {
        console.error('Erro ao pegar os metodos:', error);
        return res.status(500).json({ message: 'Internal server error' });
