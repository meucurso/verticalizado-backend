...
const deleteStudentMethods = async (studentId, editalId, areaId, methodId) => {
    try{
        const [methods] = await connection.execute("DELETE FROM student_methods WHERE studentid = ? AND editalid = ? AND areaid = ? AND methodId = ?", [studentId, editalId, areaId, methodId]);
        return methods;
    }
    catch(error){
        console.error('Erro ao inserir edital:', error);
